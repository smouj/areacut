...
if ([string]::IsNullOrWhiteSpace($OutputPath)) { $OutputPath = Join-Path $PSScriptRoot "..\assets\AreaCut.ico" }
...
try {
    ...
    $graphics.DrawImage($source, (New-Object System.Drawing.Rectangle(28, 28, 200, 200)))
    $graphics.Flush()
...
public static class AreaCutNativeMethods {
    [DllImport("user32.dll", SetLastError = true)]
    public static extern bool DestroyIcon(IntPtr handle);
}
"@
        [AreaCutNativeMethods]::DestroyIcon($iconHandle) | Out-Null
    }
} finally {
    $graphics.Dispose()
    $canvas.Dispose()
